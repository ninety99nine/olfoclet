                ...
                <div class="flex items-center justify-end">
                    ...
                    <Transition name="fade" mode="out-in">
                        ...
                        <div :key="checkedIndexes.length">
                            ...
                            <DeleteAttributeModal v-if="checkedIndexes.length > 0" :attributes="statusHandle.attributes" :indexes="checkedIndexes" mode="multiple" @deleted="resetCheckboxes()"></DeleteAttributeModal>

                            <CreateOrUpdateAttributeModal v-else :attributes="statusHandle.attributes" mode="create"></CreateOrUpdateAttributeModal>
        ...
        <div class="relative overflow-x-auto shadow-md">
            <table class="w-full text-sm text-left text-gray-500">
                ...
                <draggable
                    ...
                    v-model="statusHandle.attributes"
                    item-key="rest-api-response-status-attributes">
                    <template v-for="(element, index) in statusHandle.attributes">
                            ...
                            <td scope="row">
                                <div class="flex justify-end">
                                    <div class="opacity-0 group-hover:opacity-100 transition-all duration-500 flex items-center justify-end mr-4">
                                        ...
                                        <!-- Update Modal Button -->
                                        <CreateOrUpdateAttributeModal :attributes="statusHandle.attributes" :attribute="element" :index="index" mode="update"></CreateOrUpdateAttributeModal>

                                        <!-- Clone Modal Button -->
                                        <CreateOrUpdateAttributeModal :attributes="statusHandle.attributes" :attribute="element" mode="clone"></CreateOrUpdateAttributeModal>

                                        <!-- Delete Modal Button -->
                                        <DeleteAttributeModal :attributes="statusHandle.attributes" :attribute="element" :index="index" mode="single"></DeleteAttributeModal>
            ...
            <DefaultError :error="form.errors.status_handle_attributes" class="p-2"></DefaultError>
...
<script>
    ...
    import DeleteAttributeModal from "./DeleteAttribute/DeleteAttributeModal";
    import DefaultSearchBar from "@components/SearchBar/DefaultSearchBar";
    import CreateOrUpdateAttributeModal from './CreateOrUpdateAttribute/CreateOrUpdateAttributeModal';

    export default {
        props: ['form', 'statusHandle'],
        components: { draggable: VueDraggableNext, DefaultError, TextEditor, HiddenCode, DefaultSearchBar, CreateOrUpdateAttributeModal, DeleteAttributeModal },
        data(){
            return {
                headers: ['Name', 'Value'],
        ...
        methods: {
            startSearch(searchTerm) {
                ...
                this.statusHandle.attributes.map((attribute, index) => {
                    ...
                    //  Check if the attribute name matches the search term
                    const nameMatchesSearchTerm = attribute.name.toLowerCase().includes(searchTerm);

                    //  Check if the attribute text value matches the search term
                    const textValueMatchesSearchTerm = (attribute.value.code_editor_mode == false) && (attribute.value.text || '').toString().toLowerCase().includes(searchTerm);

                    //  Check if the attribute code value matches the search term
                    const codeValueMatchesSearchTerm = (attribute.value.code_editor_mode == true) && (attribute.value.code_editor_text || '').toString().toLowerCase().includes(searchTerm);
            ...
            toggleMultipleCheckmarks(event){
                ...
                if( event.target.checked ) {
                    ...
                    this.statusHandle.attributes.forEach((attribute, index) => this.checkedIndexes.push(index));
