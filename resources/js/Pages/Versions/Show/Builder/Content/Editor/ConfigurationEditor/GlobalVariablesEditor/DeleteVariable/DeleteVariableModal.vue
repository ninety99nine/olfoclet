<template>
    ...
    <DefaultModal
        ...
        :dangerAction="deleteVariable">

        <!-- Modal Title -->
        <template v-slot:title>Delete Variable</template>

        <!-- Modal Content (If we are deleting single variable) -->
        <p v-if="mode == 'single' && variable" class="text-sm text-gray-500 mb-5">Are you sure you want to delete the <span class="text-blue-500 font-bold">{{ variable.name }}</span> variable. After this variable is deleted you cannot recover it again.</p>

        <!-- Modal Content (If we are deleting multiple variable) -->
        <p v-else-if="mode == 'multiple'" class="text-sm text-gray-500 mb-5">Are you sure you want to delete <span class="text-blue-500 font-bold">{{ indexes.length }}</span> {{ indexes.length == 1 ? 'variable' : 'variables' }}. After {{ indexes.length == 1 ? 'this variable is deleted you cannot recover it again' : 'these variables are deleted you cannot recover them again' }}.</p>
        ...
        <template v-slot:trigger>
            ...
            <Transition name="fade">
                ...
                <div v-if="mode == 'multiple' && indexes.length" class="flex items-center justify-end">
                    ...
                    <span class="text-gray-500 text-xs mr-4">{{ indexes.length + (indexes.length == 1 ? ' variable' : ' variables') }} selected</span>
