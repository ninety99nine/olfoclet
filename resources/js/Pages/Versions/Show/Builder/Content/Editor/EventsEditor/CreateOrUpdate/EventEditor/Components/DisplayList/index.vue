    ...
    <div>
        ...
        <PrimaryAlert class="mb-4">
            <span class="text-justify">
                Review displays available for linking
            </span>
        </PrimaryAlert>
        ...
        <div>
            ...
            <!-- Displays -->
            <template v-for="(minifiedDisplay, index) in minifiedDisplays" :key="index">
                <div v-if="searchIndexes.includes(index)" @click.stop="copyToClipboard(index)"
                    ...
                    <!-- Copy To Clipboard -->
                    <CopyToClipboard :ref="'clipboard-'+index" :value="minifiedDisplay.id" message="Display ID Copied" class="absolute right-2"></CopyToClipboard>
                    ...
                    <div class="grid grid-cols-2">
                        ...
                        <div class="col-span-1 inline-flex items-center">
                            ...
                            <h1 class="text-xs font-semibold text-gray-800">{{ minifiedDisplay.name }}</h1>
                        </div>

                        <!-- ID Code Snippet -->
                        <div class="col-span-1 flex items-center justify-end">
                            <span class="text-xs text-gray-300 mr-2">ID:</span>
                            <DefaultCodeSnippet>{{ minifiedDisplay.id }}</DefaultCodeSnippet>
                        </div>
            ...
            <div v-if="hasResults == false" class="flex items-center bg-gray-50 p-8">
                <span class="text-gray-500 text-xs">No Displays</span>
            </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            minifiedDisplays() {
                return this.useVersionBuilder.screenDisplays.map((display) => {
                    return {
                        id: display.id,
                        name: display.name
                    }
        ...
        methods: {
            startSearch(searchTerm) {
                ...
                this.minifiedDisplays.map((minifiedDisplay, index) => {
                    ...
                    const nameMatchesSearchTerm = minifiedDisplay.name.toLowerCase().includes(searchTerm);

                    //  Check if the id matches the search term
                    const idMatchesSearchTerm = minifiedDisplay.id.toLowerCase().includes(searchTerm);
