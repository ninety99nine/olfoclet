<template>
    ...
    <DefaultModal
        ...
        :dangerAction="deleteAttribute">

        <!-- Modal Title -->
        <template v-slot:title>Delete Attribute</template>

        <!-- Modal Content (If we are deleting single attribute) -->
        <p v-if="mode == 'single' && attribute" class="text-sm text-gray-500 mb-5">Are you sure you want to delete the <span class="text-blue-500 font-bold">{{ attribute.name }}</span> attribute. After this attribute is deleted you cannot recover it again.</p>

        <!-- Modal Content (If we are deleting multiple attribute) -->
        <p v-else-if="mode == 'multiple'" class="text-sm text-gray-500 mb-5">Are you sure you want to delete <span class="text-blue-500 font-bold">{{ indexes.length }}</span> {{ indexes.length == 1 ? 'attribute' : 'attributes' }}. After {{ indexes.length == 1 ? 'this attribute is deleted you cannot recover it again' : 'these attributes are deleted you cannot recover them again' }}.</p>
        ...
        <template v-slot:trigger>
            ...
            <Transition name="fade">
                ...
                <div v-if="mode == 'multiple' && indexes.length" class="flex items-center justify-end">
                    ...
                    <span class="text-gray-500 text-xs mr-4">{{ indexes.length + (indexes.length == 1 ? ' attribute' : ' attributes') }} selected</span>
