<template>
    ...
    <DefaultModal
        ...
        :dangerAction="deleteParam">

        <!-- Modal Title -->
        <template v-slot:title>Delete Param</template>

        <!-- Modal Content (If we are deleting single param) -->
        <p v-if="mode == 'single' && param" class="text-sm text-gray-500 mb-5">Are you sure you want to delete the <span class="text-blue-500 font-bold">{{ param.name }}</span> param. After this param is deleted you cannot recover it again.</p>

        <!-- Modal Content (If we are deleting multiple param) -->
        <p v-else-if="mode == 'multiple'" class="text-sm text-gray-500 mb-5">Are you sure you want to delete <span class="text-blue-500 font-bold">{{ indexes.length }}</span> {{ indexes.length == 1 ? 'param' : 'params' }}. After {{ indexes.length == 1 ? 'this param is deleted you cannot recover it again' : 'these params are deleted you cannot recover them again' }}.</p>
        ...
        <template v-slot:trigger>
            ...
            <Transition name="fade">
                ...
                <div v-if="mode == 'multiple' && indexes.length" class="flex items-center justify-end">
                    ...
                    <span class="text-gray-500 text-xs mr-4">{{ indexes.length + (indexes.length == 1 ? ' param' : ' params') }} selected</span>
