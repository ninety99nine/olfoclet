<template>
    ...
    <DefaultModal
        ...
        :dangerAction="deleteHeader">

        <!-- Modal Title -->
        <template v-slot:title>Delete Header</template>

        <!-- Modal Content (If we are deleting single header) -->
        <p v-if="mode == 'single' && header" class="text-sm text-gray-500 mb-5">Are you sure you want to delete the <span class="text-blue-500 font-bold">{{ header.name }}</span> header. After this header is deleted you cannot recover it again.</p>

        <!-- Modal Content (If we are deleting multiple header) -->
        <p v-else-if="mode == 'multiple'" class="text-sm text-gray-500 mb-5">Are you sure you want to delete <span class="text-blue-500 font-bold">{{ indexes.length }}</span> {{ indexes.length == 1 ? 'header' : 'headers' }}. After {{ indexes.length == 1 ? 'this header is deleted you cannot recover it again' : 'these headers are deleted you cannot recover them again' }}.</p>
        ...
        <template v-slot:trigger>
            ...
            <Transition name="fade">
                ...
                <div v-if="mode == 'multiple' && indexes.length" class="flex items-center justify-end">
                    ...
                    <span class="text-gray-500 text-xs mr-4">{{ indexes.length + (indexes.length == 1 ? ' header' : ' headers') }} selected</span>
