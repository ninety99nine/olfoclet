<template>
    ...
    <DefaultModal
        ...
        :dangerAction="deleteValidationRule">

        <!-- Modal Title -->
        <template v-slot:title>Delete Validation Rule</template>

        <!-- Modal Content (If we are deleting single rule) -->
        <p v-if="mode == 'single' && rule" class="text-sm text-gray-500 mb-5">Are you sure you want to delete the <span class="text-blue-500 font-bold">{{ rule.name }}</span> validation rule. After this validation rule is deleted you cannot recover it again.</p>

        <!-- Modal Content (If we are deleting multiple rule) -->
        <p v-else-if="mode == 'multiple'" class="text-sm text-gray-500 mb-5">Are you sure you want to delete <span class="text-blue-500 font-bold">{{ indexes.length }}</span> {{ indexes.length == 1 ? 'validation rule' : 'validation rules' }}. After {{ indexes.length == 1 ? 'this validation rule is deleted you cannot recover it again' : 'these validation rules are deleted you cannot recover them again' }}.</p>
...
<script>
    ...
    export default {
        ...
        methods: {
            deleteValidationRule(closeModal) {
