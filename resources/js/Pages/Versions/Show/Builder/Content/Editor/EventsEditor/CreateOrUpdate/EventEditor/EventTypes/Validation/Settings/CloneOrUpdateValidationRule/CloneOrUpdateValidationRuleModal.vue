<template>
    ...
    <DefaultModal
        ...
        @open="resetForm()"
        @close="$emit('close')"
        :primaryAction="submitValidationRule"
        :defaultAction="cancelCreateValidationRule"
        :primaryText="modeInCaps + ' Validation Rule'">

        <!-- Modal Title -->
        <template v-slot:title>{{ modeInCaps + ' Validation Rule' }}</template>
        ...
        <PrimaryAlert v-if="form.type == 'custom_code'" class="mb-6">
            <span class="text-justify">
                ...
                <span class="font-semibold text-green-500">True</span>
                to pass validation or
                <span class="font-semibold text-red-500">False</span>
                to fail validation.
            </span>
        </PrimaryAlert>
        ...
        <!-- Regex Pattern -->
        <TextOrCodeEditor v-else-if="form.type == 'custom_regex'" v-model="form.rule" label="Regex Pattern" :error="form.errors.rule" class="mb-6"></TextOrCodeEditor>
        ...
        <!-- Error Message -->
        <TextOrCodeEditor v-model="form.error_msg" label="Message" note="Validation message on fail" :error="form.errors.error_msg" class="mb-6"></TextOrCodeEditor>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            submitValidationRule(closeModal) {

                //  Clear existing errors
                this.form.clearErrors();

                //  Check if we have an existing rule using the same name
                const totalExactMatches = this.useVersionBuilder.searchValidationRules(this.rules, this.form.name, true).length;
